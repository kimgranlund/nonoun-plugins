...
Treat brand-studio as a pattern library for plugin authors. The five-primitive shapes it demonstrates and that future plugins can copy:
...
brand-studio was authored, validated, and red-teamed via **`plugins-studio`** — the plugin lifecycle studio that carves components, wires `plugin.json` / `marketplace.json`, validates structure, and runs the adversarial plugin-architecture review. Self-contained by design: zero cross-plugin dependencies.
