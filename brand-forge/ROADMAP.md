...
- [v0.5] ~~**A multi-lens Muse fan-out.**~~ **DECLINED 2026-06-17 — keep the single Muse.** The proposal was to promote the Muse's internal lenses to parallel isolated agents (ideal · contrarian · analogical · principles) like the council. Assessed and declined: the single `brand-muse` agent is **already calibrated** (Halcyon 6/6 × N=3) and navigates all the lenses correctly, so the fan-out's premise — that parallel isolation adds signal over the single agent — is **unproven**, and it would add ~5 always-on agent descriptions for a speculative benefit. That is the symmetry-for-symmetry the maker-agent item warns against, applied to the Muse; this item gated itself on "*if* aspiration diversity proves as valuable as critic diversity," and it hasn't. Revisit only if the single Muse demonstrably misses lens-independence in real use (validate the hypothesis on a lens-independence fixture *before* building the agents).
- [v0.5] **Making roles as optional maker agents.** The Team's seats are methodology knowledge, adopted on demand within `/brand-build`. **First seat promoted 2026-06-17 (0.4.23): `brand-copywriter`** — the Copywriter earns an isolated agent for *extended* voice work (a full platform / naming set / long copy run); `/brand-build` delegates to it, brief copy stays in-methodology, the voice sub-council reviews it cold. The other seats stay in-methodology by design (brief lenses or integrative work, not long single-context) — promoting them would be the symmetry-for-symmetry this item forbids. **Open:** further promotions only if a seat proves the same long-context need; a dedicated generation-calibration for `brand-copywriter` (as the Muse got post-ship).
