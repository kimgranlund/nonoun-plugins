...
**The brand-decomposer seam (next increment):** the assembler will additionally project a `*.brand.json` card for grading + operability-checking by `design-skills:brand-decomposer` when installed (optional; brand-forge stays self-contained).
