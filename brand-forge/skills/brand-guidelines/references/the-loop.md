...
**Location:** the ledger lives in the corpus so the assembled docs can `sources:`-cite it — **`00-sources/guidelines-elicitation.json` under the default corpus root `./brand-corpus`** (the same default `/brand-corpus-export` + `/brand-stack` use; pass a path to override). It is evidence — the choices that produced the guidelines. Append-only; a revision sets `supersedes` to the earlier entry id (never rewrite a decision — the corpus write-discipline). Validate after every write: `guidelines-ledger validate` (no arg → the default ledger).
