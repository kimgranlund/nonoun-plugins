...
2. **Read the ground truth first.** Invoke the **`brand-corpus`** skill (+ the corpus MCP if `corpus_dir` is set) to read `01-foundation` and the existing choice-ledger (`00-sources/guidelines-elicitation.json`) — every option must trace to the decided foundation; resume at the uncovered frontier.
3. **Run one 2×2 round.** Frame the domain's two axes → generate four design-move cards grounded in the foundation + a cited exemplar (name the brand + mechanism, never copy assets) + the quality bar → present the 2×2 + cards → capture the pick + comments as a typed choice → drill down or advance.
...
Assembly into corpus docs + the brand-decomposer card-projection seam are later increments — for now, build the ledger. A card you can only write by *asserting* rather than tracing it to `01-foundation` has surfaced an undone foundation: say so, and fix the foundation (`/brand-build`) first.
