...
>
> Concretely: working memory = entry files (AGENTS.md / CLAUDE.md). Long-term memory = ADRs (episodic decisions) + post-mortems (procedural lessons) + CHANGELOG (autobiographical timeline). Autobiographical introspection = `.agents/brain/audit-history/`. Autonomic functions = pre-commit hooks + scheduled CI. Habit formation = the Anthropic iterate pattern. The repo gets _richer_ over time as humans deposit and curate; the agents read what's there.
...
| "Set up .agents/brain folder" / "migrate to .agents/brain" | Greenfield vs migration | "Initialize the v1.5 layout, or migrate from `docs/`?" |
...
| Orphan detection | Graph-reachability from entry files; `find .agents/brain/ docs/ -name "*.md"` |
...
| Memory-home completeness | `.agents/brain/adrs/` (or `docs/adrs/`) has ≥1 entry ≤ 90 days old **OR** an explicit "no architectural changes in window" note |
...
| Running a full audit (incl. `.agents/brain/` migration) | `references/recipes/audit-existing-repo.md` |
| Greenfield setup (`.agents/brain/` defaults) | `references/recipes/greenfield-setup.md` |
...
| **Reliability dial** + git-sync — `.agents/brain/config.toml` strictness AND `shared`/`local-only` mode | `references/guidance/reliability-dial.md` |
...
| --- | --- | --- |
| Less-wasteful | (a) `orphan-detection` finds zero unreferenced files in `.agents/brain/` and `docs/` (archived ones live in `.agents/brain/archive/`). (b) MD5(CLAUDE.md) ≠ MD5(AGENTS.md) only if symlink/thin-pointer. | Pre-commit hook + weekly CI |
| Token-optimized | `wc -l AGENTS.md CLAUDE.md` < 200 each (warn at 150). Token-waste detection across `.agents/brain/` + `docs/` finds zero >500-line files that aren't ADRs/postmortems. | Pre-commit hook |
| Stale-resistant | `lychee` exits clean. Every file in `.agents/brain/` and `docs/` has a `date:` frontmatter or `_Last reviewed:_` line within 365 days. | PR CI + scheduled CI |
| Self-healing | The hooks above are wired AND have **fired recently**. The audit verifies **presence** (hooks/workflows exist) AND **liveness** — `${CLAUDE_PLUGIN_ROOT}/bin/audit-history.py liveness --base <repo> --strictness <dial>` checks for an audit record in `.agents/brain/audit-history/` within the dial's freshness window (lax 90d / normal 30d / strict 8d). Present-but-never-fired → `STALE TRIP-WIRE` (exit 1); absent → `MISSING TRIP-WIRE` (exit 1). Presence without liveness is clean-by-luck, not self-healing. | `${CLAUDE_PLUGIN_ROOT}/bin/audit-history.py liveness` (presence + last-fired recency) |
| Continuously-learning | (a) `.agents/brain/adrs/` and `.agents/brain/postmortems/` (or legacy `docs/` equivalents) exist; both have ≥1 entry from the last 90 days OR an explicit "no architectural changes / no incidents in window" note. (b) `.agents/skills/` or `.claude/skills/` exists; `audit-skills` runs clean (no errors); skill frontmatter passes `check-skill-frontmatter`. | Quarterly review + skill audit |
...
- **Specific fixes**: edits to `AGENTS.md` / `CLAUDE.md` / other entry files; new files to seed (`.agents/brain/adrs/{yyyy-mm-dd}-{decision-summary}.md`); removals of stale duplicates.
...
- [ ] **Layout identified** _[review]_ — `.agents/brain/` (v1.5+), legacy `docs/`, or both? Routes to different recipe sections.
...
- [ ] **Tool stack identified** _[review]_ — resolve link-check / hooks / CI / skill-audit commands through `[repo-ops.tools]` in `.agents/brain/config.toml` (defaults assume Node + GitHub; override for Python / GitLab / no-skills). Recipe commands (`lychee`, `npm run …`, `.github/workflows/`) are defaults, not assumptions.
...
3. **Entry files explicitly point to subfolders.** A naked `AGENTS.md` is incomplete — it must say "for architecture decisions, read `.agents/brain/adrs/`", "for the roadmap, read `PLAN.md`", "for change history, read `CHANGELOG.md`."
4. **Memory primitives have homes.** ADRs in `.agents/brain/adrs/` (numbered, dated, status). **Postmortems in `.agents/brain/postmortems/`** (Google SRE blameless format). Runbooks in `.agents/brain/runbooks/`. Each gets a pointer from `AGENTS.md`. Greenfield repos default to `.agents/brain/` (since v1.5); existing repos with `docs/{adrs,postmortems,...}/` are recognized by the audit and may migrate via `recipes/audit-existing-repo.md` § Migration.
5. **Every doc is dated.** YAML frontmatter `date:` or `_Last reviewed: YYYY-MM-DD_`. Staleness must be visible.
6. **Orphans are a smell.** Files in `.agents/brain/` or `./docs/` that no entry file references are either (a) missing from the index or (b) abandoned and should be archived.
...
| **Orphan detection** | Files in `.agents/brain/` or `./docs/` not referenced by any entry file | `references/audit-patterns/orphan-detection.md` |
...
| **Memory fragmentation** | No `.agents/brain/adrs/`; ADRs in random places; decisions mixed with code comments | `references/audit-patterns/memory-fragmentation.md` |
...
| **Archive-link sweep** | After cross-tree doc moves (e.g. `docs/X.md` → `.agents/brain/archive/Y.md`), every linker needs depth-aware path rewrite | `references/audit-patterns/archive-link-sweep.md` |
...
| `.agents/brain/adrs/` | Numbered ADRs (the _canonical_ "Architecture Decision Log") | YES |
| `.agents/brain/postmortems/` | Blameless incident write-ups (Google SRE / Atlassian) | YES if exists |
| `.agents/brain/runbooks/` | Operational procedures | YES if exists |
| `.agents/brain/archive/` | Superseded/abandoned docs (auto-archived after 30-day grace) | n/a |
| `.agents/brain/architecture/` | Diagrams + extended docs (short overview is `ARCHITECTURE.md` at root) | YES if exists |
| `.agents/brain/audit-history/` | JSON audit ledger (queryable, SOC2-friendly) | n/a |
| `.agents/brain/changesets/` | AGENTS.md changesets staging (multi-contributor; consolidated at merge) | n/a |
| `.agents/brain/cache/` | WebFetch cache for external-ref-verification (gitignored) | n/a |
| `.agents/brain/cold-start/` | Harvest-session working state (gitignored) | n/a |
| `.agents/brain/config.toml` | Repo-brain strictness configuration (lax/normal/strict) | n/a |

> **Legacy `docs/` layout supported.** Repos pre-dating v1.5 may have `docs/adrs/`, `docs/postmortems/`, `docs/runbooks/`, `docs/archive/`, `docs/architecture/`. The audit recognizes both. Migration to `.agents/brain/` is opt-in via one `git mv` (recipe in `recipes/audit-existing-repo.md` § Migration).
...
4. **Memory primitives have homes (v1.5 layout).** ADRs in `.agents/brain/adrs/`, postmortems in `.agents/brain/postmortems/`, runbooks in `.agents/brain/runbooks/`, archive in `.agents/brain/archive/`. Audit recognizes legacy `docs/{adrs,postmortems,...}/` and nudges toward migration.
