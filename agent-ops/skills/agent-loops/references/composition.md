...
| **Cron / scheduled agents** | The cron expression or trigger; the entrypoint command; the durable state path the agent reads on resume and writes before exit. Example: `0 6 * * * python3 agent.py --state .brain/audit-state.json`. Requires external durability; the agent must be idempotent. |
