...
| **Agents** | `agents/*.md` (11) | The operating roster — `harness-advancer · harness-auditor · harness-distiller` (the human + the commands are the controller; a `/harness-run` auto-loop orchestrator is ROADMAP) — plus the **structural council**: `harness-council` (orchestrator) + 7 `critic-*` lenses, one per rubric dimension (H1–H7) | One cell per worker dispatch, fresh context; critics run `Read/Grep/Glob`-only in parallel isolated contexts (structural, *not* named-practitioner — that lens is agent-ops's) |
...
### The kernel (`bin/`) — the deterministic core (11 scripts, all selftested)

`lattice.py` is THE kernel — `scan · rank · validity · advance · check · scaffold` plus the maturity state machine and staleness-as-graph-computation; `init` scaffolds the nine layer dirs + `signals/` + `ledger/` + the naming schema, idempotency-guarded; it carries a `KERNEL_VERSION` and an atomic `save()`. `ledger.py` is append-only provenance + the loops it closes (probe-cost → compass, false-pass → trust, distill → regeneration) + `no-progress` (the failure-loop detector, in code — CV4); `false-pass` returns **`unmeasured`**, not a misleading 0.0%, until an independent refuter exists. `naming.py` validates typed names against `schemas/naming.schema.json` and **dogfoods** the real `agents/*.md` + the plugin name. `validate.py` is the **validation path** (below); `wire.py` is the **consent-gated installer** whose `check` now **fails (not warns) on a drifted wired-kernel copy** (CV1). The three gateverb species the grammar pre-names: `gate-signal` (PreToolUse deny, segment-anchored, `.harness/`-scoped), `emit-ledger` (PostToolUse audit trail), `propagate-staleness` (PostToolUse staleness cascade). `council-precheck.py` is the council's one-call Step-0 driver (shrinks the orchestrator's Bash surface). `harness-hook` is the advisory session hook; `lattice-mcp.py` is the read perimeter. The four `schemas/` (`cell · lattice · naming · signal`) are the typed data — and `lattice.py check` now reads its enums **from `cell.schema.json`** (the single source) + flags unknown keys (a full discriminated-union schema is ROADMAP).
...
CI runs three behavioral gates beyond the selftests: `evals/calibration/check_fixtures.py` (four planted-defect `.harness/` fixtures — retro partial-order, unwired gate, unearned autonomy, stale-but-trusted — each **caught by the right `bin/` gate**, with a freshly seeded + wired project as the passing directionality control), `check_baselines.py` (five recorded `harness-evaluate` baselines, concept-recall-gated: each names its planted headline + cap; the embedded injection is quoted as a finding, never obeyed; the positive control reads sound), and `evals/first-slice-walkthrough/replay.py` (drives the whole loop end to end with only public machinery and asserts the closed end state — it caught the seed's circular-verifier deadlock at authoring time). The calibration answer key lives in `evals/calibration/README.md`, **never inside a fixture** — judge runs against fixtures must stay cold.
