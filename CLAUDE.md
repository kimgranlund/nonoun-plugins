...
| **Hook + gate** | `hooks/hooks.json` + `bin/harness-hook`, `bin/gate-signal` | Advisory naming/staleness lint (hook); blocking protected-verifier deny (gate) | The hook **never blocks** (always exit 0); the gate blocks but is **user-wired** (below) |
