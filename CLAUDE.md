...
`lattice.py` is THE kernel — `scan · rank · validity · advance · check · scaffold · block/unblock` plus the maturity state machine and staleness-as-graph-computation; `init` scaffolds the nine layer dirs + `signals/` + `ledger/` + the naming schema, idempotency-guarded; it carries a `KERNEL_VERSION` and an atomic `save()`; `block` flips the protected budget/no-progress stop flag (a blocked cell drops out of `rank`). `ledger.py` is append-only provenance + the loops it closes (probe-cost → compass, false-pass → trust, distill → regeneration) + `no-progress` (the failure-loop detector, in code); `false-pass` returns **`unmeasured`**, not a misleading 0.0%, until an independent refuter exists. `naming.py` validates typed names against `schemas/naming.schema.json` and **dogfoods** the real `agents/*.md` + the plugin name. `validate.py` is the **validation path**; `wire.py` is the **consent-gated installer** whose `check` **fails (not warns) on a drifted wired-kernel copy**. The gateverb hook species the grammar pre-names: `gate-signal` (PreToolUse deny on verifier assets), **`gate-budget`** (PreToolUse deny on a write to a `blocked` cell — the wired stop-gate that bounds the autonomous loop), `emit-ledger` (PostToolUse audit trail), `propagate-staleness` (PostToolUse staleness cascade). `council-precheck.py` is the council's one-call Step-0 driver. `harness-hook` is the advisory session hook; `lattice-mcp.py` is the read perimeter. The four `schemas/` (`cell · lattice · naming · signal`) are the typed data — and `lattice.py check` reads its enums **from `cell.schema.json`** (the single source) + flags unknown keys (a full discriminated-union schema is ROADMAP). **The bounded loop:** `/harness-run` → the `harness-builder` orchestrator runs `rank → advance → no-progress → block` under hard caps. The caps are enforced **in code, not the agent's counting** (the v0.4.1 fix for the council's convergent Critical): `run-budget.py` persists the run budget and the wired `gate-budget` denies *every* write once `now ≥ deadline` or the ledger-counted iterations/cells hit the cap (`evals/global-bound/` proves it with no model agent); per-cell, `ledger.py no-progress` → `lattice.py block` → `gate-budget` deny (`evals/stop-gate/`). `harness-status.py` is the cheap operator dashboard (maturity · frontier · run budget · wiring · gate-fires · ledger); `block`/`unblock` ledger their decision.
