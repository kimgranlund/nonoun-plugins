...
| Primitive | Location | Job | Invariant |
| --- | --- | --- | --- |
| **Commands** | `commands/*.md` (8) | Thin typed entry points — `/harness-seed · scan · next · advance · run · distill · audit · council` | Route to skills + `bin/`; never re-contain the model |
...
CI runs four behavioral gates beyond the selftests: `evals/calibration/check_fixtures.py` (four planted-defect `.harness/` fixtures — retro partial-order, unwired gate, unearned autonomy, stale-but-trusted — each **caught by the right `bin/` gate**, with a freshly seeded + wired project as the passing directionality control), `check_baselines.py` (recorded `harness-evaluate` baselines, concept-recall-gated: each names its planted headline + cap; the embedded injection is quoted as a finding, never obeyed), `evals/first-slice-walkthrough/replay.py` (drives the whole engine loop end to end with only public machinery — it caught the seed's circular-verifier deadlock at authoring time), and `evals/stop-gate/check.py` (proves the **wired stop-gate halts a runaway loop**: a repeated-failure cell is detected by `ledger.py no-progress`, blocked, dropped from `rank`, and denied by `gate-budget` — three independent code mechanisms). The calibration answer key lives in `evals/calibration/README.md`, **never inside a fixture** — judge runs against fixtures must stay cold.
