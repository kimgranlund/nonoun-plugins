...
| **H6 Autonomy tier** `[review]` `[hypothesis]` | the operating tier is ≤ the **earned ceiling** the measured false-pass rate supports (< ~5% gates unattended) + zero reward-hacking | **`ledger.py trust`** (the earned-tier ceiling, computed — promotion advisory, demotion auto) over `ledger.py false-pass`; the trust trajectory in `layer-policy.md` |
