...
| **H3 Anti-reward-hacking** `[gate]` | verifier assets deny-on-write to workers; signals minted by `validate.py` from an external exit status, not hand-asserted; ≥1 pristine-reference check | `gate-signal` is **wired** as a PreToolUse deny in the loop config (present-but-unwired = false pass); worker carries no `Bash`; exploit scan of passing runs |
