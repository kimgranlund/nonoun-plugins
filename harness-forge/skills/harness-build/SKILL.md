...
- **Global** — `bin/run-budget.py start` persists the run's caps; the wired `gate-budget` then **denies every worker write** once the run is exhausted: `now ≥ deadline` (wall-clock — an absolute deadline, no counter), or ledger-counted `validate` events ≥ max-iterations, or validated-cells ≥ max-cells. The loop *physically cannot write past its budget*, whatever the orchestrator believes its counter says. (`evals/global-bound/` proves it with no model agent.)
