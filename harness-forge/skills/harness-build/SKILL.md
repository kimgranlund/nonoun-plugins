...
- **Global** — `bin/run-budget.py start` persists the run's caps (the orchestrator's **step 0** — this *arms* the gate); the wired `gate-budget` then **denies every worker write** once the run is exhausted: `now ≥ deadline` (wall-clock — an absolute deadline, no counter), or ledger-counted `validate` events ≥ max-iterations, or validated-cells ≥ max-cells. **Once armed, the loop physically cannot write past its budget**, whatever the orchestrator believes its counter says (`evals/global-bound/` proves it via the CLI, no model agent). The precondition stays honest: a cap-less budget is refused, `/harness-status` alarms when none is active, and `budget.json` is deny-on-write to workers — but *arming* is the orchestrator's step 0, so an un-armed run is unbounded (say which state you're in).
