...
run-budget.py start --max-iterations N --max-cells M --wall-clock-s S
   → persists .agents/harness/run/budget.json {start_ts, deadline_ts, max_iterations, max_cells}
...
The deny fires from the **installed** `.agents/harness/hooks/gate-budget` (the wired location), end to end, with no orchestrator in the loop — the same standard `evals/stop-gate/` meets for the per-cell breaker.
