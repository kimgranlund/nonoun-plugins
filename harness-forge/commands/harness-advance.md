...
On a passing signal, mark the cell `validated` and rescan (validation reveals new gaps). On budget exhaustion or a no-progress signature (`bin/ledger.py no-progress` computes the latter from the ledger — exit 1 means halt), flip `blocked` (`lattice.py block`) and surface to the compass rather than burning tokens; in a wired project the `gate-budget` hook then denies further writes to that cell. The worker does not declare its own completion; a separate done-judge does. For the automated form of this loop across the whole frontier, see `/harness-run`.
