...
4. **Offer to wire the blocking gates into this project's worker loop — consent-gated, never silent.** Run `python3 "${CLAUDE_PLUGIN_ROOT}/bin/wire.py" plan` and show the user exactly what would change (three hook copies into `.harness/hooks/` + three entries merged into the project's own `.claude/settings.json`: the PreToolUse `gate-signal` deny on verifier assets, and the PostToolUse `emit-ledger` audit trail + `propagate-staleness` cascade). Only on the user's explicit OK, run `wire.py apply`, then prove it with `wire.py check` (exit 0 = wired). If they decline, note that the anti-reward-hacking protection stays conventional until wired — `wire.py apply` can run any time, and `wire.py unwire` reverses it exactly.
