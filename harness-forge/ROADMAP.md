...
- ~~**`/harness-run` + the orchestrator's return + the wired stop-gate.**~~ **Shipped in 0.4.0** (per-cell breaker) **+ 0.4.1** (the GLOBAL caps enforced in code: `run-budget.py` + `gate-budget` deny every write once the run is exhausted — wall-clock/iterations/cells, computed from the deadline + ledger, no agent counter; `evals/global-bound/` proves it). Still open from this line: a **token/dollar** budget meter (today's caps are iterations/cells/wall-clock; token accounting needs the ledger's `cost` summed against a ceiling — the run-budget machinery is the natural home), and **trust-trajectory automation** (auto-promote/demote a loop family by the measured false-pass rate — today the tier is read at the boundary, set by hand).
...
- ~~**Post-deploy observability (Charity M.).**~~ **Shipped** — `/harness-status` (0.4.1) + the run-budget X/Y line, the unbounded alarm, and the gate-fire counter (0.4.2). Still open from this line: a **streaming** signal (today `/harness-status` is a manual pull; a long unattended run emits nothing to watch in real time — the PostToolUse hooks could be the spine of a tail-able progress line), and a **run-budget liveness marker** (a crashed run leaves a stale `budget.json` that wrongly bounds the next work until `clear`; a PID/heartbeat + a startup reconciliation would self-heal it).
