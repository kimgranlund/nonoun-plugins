...
- **The arming step, mechanized (Chip/Andrej, v0.4.1 council).** The global bound is *enforced* in code but *armed* by the orchestrator calling `run-budget.py start` (step 0); skip it and the run is unbounded (0.4.2 scopes the claim, refuses a vacuous budget, and alarms — but the gate can't fail-closed without a budget, since it can't tell a loop-write from manual editing). A true fix needs a loop-entry signal the gate can read ("a `/harness-run` is in flight") so a write during an un-budgeted loop is denied — e.g., `/harness-run` writes a run-active marker the gate treats as "must have a budget."
