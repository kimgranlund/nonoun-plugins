...
- ~~**`/harness-run` + the orchestrator's return + the wired stop-gate (one release).**~~ **Shipped in 0.4.0** — `/harness-run` + the restored bounded `harness-builder` + the wired `gate-budget` stop-gate (detect → block → deny, three independent code mechanisms); `evals/stop-gate/` proves the breaker breaks. Still open from this line: a **token/dollar** budget meter (today's caps are iterations/cells/wall-clock; per-cell token accounting needs the ledger's `cost` summed against a ceiling), and **trust-trajectory automation** (auto-promote/demote a loop family by the measured false-pass rate — today the tier is read at the boundary, set by hand).
