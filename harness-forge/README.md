...
├── agents/     3 operators + the council    → harness-advancer · harness-auditor · harness-distiller · harness-council + 7 critic-* structural critics (one per rubric dimension, parallel isolated)
...
- **v0.3 is the kernel + the operating roster + the wired loop + the structural council, validated, selftested, and behaviorally evidenced.** CI replays the whole loop end to end (`evals/first-slice-walkthrough/`) and proves the gates catch the planted defect classes while a clean control passes (`evals/calibration/`, recall-gated). A `harness-run` auto-loop command (+ the orchestrator that drives it) with a **wired** budget/no-progress stop-gate, the council-as-separate-plugin, verifier adapters, and family **kits** are on the [ROADMAP](ROADMAP.md).
