...
- **The loop is bounded in code, not by the orchestrator's discipline** — `/harness-run` runs the `harness-builder` orchestrator across the frontier under hard caps, and the caps are **enforced by the wired `gate-budget`**, not counted in the agent's context: `run-budget.py` persists the run's budget, and the gate denies *every* write once `now ≥ deadline` (wall-clock), or the ledger-counted iterations/cells hit the cap. The loop *physically cannot write past its budget* (`evals/global-bound/` proves it with no model agent). Per-cell, `ledger.py no-progress` detects a stuck cell, the orchestrator blocks it, and `gate-budget` enforces the block (`evals/stop-gate/`). What stays the orchestrator's *discipline* is only the graceful stop — and even if it ignores that, the gate denies its writes. Attended by design; autonomy is **earned, not declared**.
