...
- **The loop's caps are enforced in code; arming them is the orchestrator's step 0** — `/harness-run` runs the `harness-builder` orchestrator under hard caps, and the **enforcement** is the wired `gate-budget`, not the agent's counting: `run-budget.py start` persists the run's budget (step 0), and from then the gate denies *every* write once `now ≥ deadline` (wall-clock), or the ledger-counted iterations/cells hit the cap — **once a run is armed, the loop physically cannot write past its budget** (`evals/global-bound/` proves it with no model agent, via the orchestrator's own CLI). The honest precondition: the gate can only enforce a budget that *exists*, and the orchestrator creates it at step 0; skip that and there's no ceiling — so `run-budget.py start` refuses a cap-less budget, `/harness-status` **alarms** when none is active, and the budget file is deny-on-write to workers (a worker can't lift its own ceiling). Per-cell, `ledger.py no-progress` → block → `gate-budget` deny (`evals/stop-gate/`). Attended by design; autonomy is **earned, not declared**.
...
- **v0.4 is the kernel + the operating roster + the wired loop + the bounded autonomous loop + the structural council, validated, selftested, behaviorally evidenced, and thrice dogfood-red-teamed.** CI replays the engine loop (`evals/first-slice-walkthrough/`), proves both the per-cell stop-gate (`evals/stop-gate/`) and the **global run-budget enforced in code with no model agent** (`evals/global-bound/`, the v0.4.1 fix for the council's convergent "bounded by construction" Critical), and proves the gates catch the planted defect classes (`evals/calibration/`). The council-as-separate-plugin, the discriminated-union schema (evidence-less validation made unrepresentable), a token/dollar budget meter, trust-trajectory automation, verifier adapters, and family **kits** are on the [ROADMAP](ROADMAP.md).
