...
1. **Seed** (`/harness-seed`) — `lattice.py init` lays `.agents/harness/` + the first thin slice (an ontology + spec + rubric + ledger task slice). The ledger schema sits in the first slice; provenance cannot be retrofitted. The seed's final step **offers to wire the blocking gates** into the project's own loop (`wire.py plan` → user consent → `apply` → `check`): the PreToolUse `gate-signal` deny + the `gate-budget` stop-gate, the `emit-ledger` audit trail, the `propagate-staleness` cascade. Wire before the first unattended pass — `wire.py check` exit 0 is the precondition for claiming the protection is mechanical.
...
4. **Advance** (`/harness-advance`) — gate with `lattice.py validity`, then the `harness-advancer` runs the engine; the validation path writes the signal; `ledger.py append` records the why. **Automated form:** `/harness-run` dispatches the `harness-builder` orchestrator to run steps 2–5 in a **bounded** loop (hard caps + the wired no-progress stop-gate: a stuck cell is blocked, `gate-budget` denies further writes, the loop halts and reports). Attended until the family earns autonomy.
