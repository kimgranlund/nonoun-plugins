...
**Check**: cell `budget` fields; `ledger.py no-progress` is the detector (code); the loop is **bounded** (the `harness-builder` / `/harness-run` caps — max-cells/iterations/wall-clock) and the stop-gate is **wired** (`wire.py check` exit 0 → `gate-budget` denies writes to a blocked cell); the done-judge is a distinct path from the worker. **Wiring honesty still applies**: a harness claiming "budgets enforced mechanically" while `wire.py check` is NOT WIRED is overclaiming exactly as an unwired `gate-signal` would — the detector computes either way, but enforcement needs the wire.
