...
- **Respect the budget.** On the iteration cap, the token budget, or a repeated failure signature, stop, record why, and exit. Do not loop harder. The no-progress signature is computed, not guessed — `bin/ledger.py no-progress` flags a cell whose last N validates all failed. In a wired project you do not even get the choice: once the orchestrator blocks a stuck cell, the `gate-budget` hook **denies your next write to it** — you cannot grind a blocked cell. Honor the stop before the gate has to enforce it.
