...
- **Anti-reward-hacking** — are verifier assets (signals, rubrics, schemas, hooks) mechanically deny-on-write to workers (`bin/gate-signal`)? Is at least one check computed from pristine reference the worker cannot reach? A clean scoreboard is exactly what a hack produces — scan passing runs adversarially.
