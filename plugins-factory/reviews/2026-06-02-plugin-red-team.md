...
### Highest severity — P9/ST1 bundled trifecta (Simon W.) — FIXED
...
### Co-located dangling refs — P3/P4 (David F.) — FIXED
...
### Hook injection framing — P9/ST5 (Simon W.) — FIXED
...
- **No CI; tree git-untracked (P8, David F.).** The plugin mandates "`validate_plugin.py` in CI" three times but ships none and has no version history. The selftest / validate / foundations gates pass locally; wiring them into CI + committing is a repo-level step (flagged to the owner) — see ROADMAP.
- **No council-calibration eval — the blind spot all nine missed.** No eval tests the council's own _output_ fidelity (reproducibility across runs, severity calibration, a guard against multiply-owned dimensions inflating one issue into several "independent" findings). The plugin ships a routing eval but none for its headline capability.
- **Dimension MECE audit.** `andrej-k`/verifiability owns no single Pn; P1 and P2 are multiply-owned. The 9-dimension partition is itself unvalidated.
