...
4. **Validate** with `${CLAUDE_PLUGIN_ROOT}/bin/validate_plugin.py plugin <dir>`, then run the **build-time red-team** (Simon W. + Scott W. floor; `/plugin-critique full-panel` if it bundles a hook/MCP or is a marketplace) and fold surviving Critical/Major findings back in.
