...
- **Gates (`bin/`)** — stdlib Python, all run in CI against every catalog plugin:
  ...
  - `check-manifest-sync.py` — fails on declared-state drift (version↔CHANGELOG, description count claims, cited commands, and **enumeration completeness** — a parenthetical command list that omits a shipped command, the brand-forge "6 of 7" drift the rubric calibration surfaced, now C4).
  ...
  - `context-cost.py` — the **P6 Context-Economy always-on audit**, the dimension's mechanization (the rubric names it by hand). Measures the standing context tax a plugin charges every session: the eagerly-loaded `description`s of every agent / command / skill (bodies/references load on-invoke, so they're excluded). Reports total + est. tokens + per-category breakdown + the heaviest components; WARNs on a verbose description *dominating* the budget; hard-FAILs only on a >2KB body-in-an-advertisement or an opt-in `--budget` breach (P6 is a judgment, so it exposes cost rather than grading taste). `--with-mcp` spawns each live MCP and folds its served `tools/list` size into the total — the rubric's biggest P6 concern (a 1:1-API-wrapper MCP) — WARNing if the tool-defs dominate (executes servers, so it shares `check-mcp-liveness`'s I-12 interlock: `--with-mcp` refuses without `--trusted-source` while the default static audit stays open). It also reports a **distinct install-payload axis** (I-11) — committed (git-tracked) `bin/` weight, which ships in every install but never loads into context — and WARNs when a single `bin/` subtree dominates it (the off-job-toolbox smell the description-only meter was blind to — it's what flagged the corpus-reader for relocation, I-11/D-14, and still fires on the maker plugins that vendor it). `selftest`-proven, incl. folded-block-scalar parsing + the MCP measurement + the install-payload WARN. Catalog standing cost: ~11.6K tok static / ~12.9K tok with the four live MCPs' tool-defs (brand-corpus · product-corpus · repo-memory · lattice-query).
  - `assemble-marketplace.py` — the carve **publish** step. Discovers a carved library's plugins and assembles their multi-plugin `.claude-plugin/marketplace.json`, **failing unless the carve is legal**: every plugin manifest + the marketplace validate (incl. the cross-plugin agent-collision check), and the `dependencies` graph **resolves** (in-library or `--allow-external`-vouched) and is **acyclic** (foundation plugins are sinks — carve-quality D4). `selftest`-proven; CI round-trips the real 5-plugin catalog. Produces + validates the manifest; it doesn't deploy.
...
The **corpus-reader** (a buildless static reader for a folder-of-Markdown corpus) and its `sync-corpus-reader.py` used to live here, but they serve the **maker** plugins (brand-forge / product-forge), not the judge — so the self-red-team (I-11/D-14) relocated them to repo-root build tooling at [`tools/corpus-reader/`](../tools/corpus-reader/). plugins-factory keeps only **`bin/check-bake-safety.py`** — the path-generic XSS gate that proves a baked reader is integrity-pinned and script-context-safe (a security gate, the catalog's gate-owner's job, independent of where the reader lives).
