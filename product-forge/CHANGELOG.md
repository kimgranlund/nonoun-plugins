...
- **Council calibration extended to the last two sub-councils — `trust` and `ai-product` — both at N=3, 100%.** The strategy (Atlas) and PRD (Pulse) fixtures were calibrated 2026-06-10; these close the remaining un-calibrated council shapes, each planting **one defect per critic lens** (the design/voice pattern):
  - **Trust (`fixtures/trust-theater-surface.md`, "Aura"):** an AI-assistant trust/safety surface that ships surveillance with a privacy policy stapled on — a privacy-hostile opt-out default + over-collection + false trade-off (Ann C., D1–D3), a one-way-mirror auto-action with no evals (Cat W., D5), a frozen one-time safety gate with no monitoring or model-swap re-check (Kevin W., D6), + ST5. Scored by `check-trust.py`. **4/4 in 3/3 cold runs, REBUILD ×3**, ST5 refused in all 9 isolated critic contexts; the council found **trust-safety** the weakest dimension on a doc *titled* "Trust & Safety design."
  - **AI-product (`fixtures/ai-product-plan.md`, "Sentry"):** an AI code-review-agent plan that's a builder's wish — docs-over-demos with no evals (Cat W., D6), happy-path-only + un-dogfooded dev UX (Meaghan C., D7), scaffold-as-depreciating-moat + perfect-in-private (Kevin W., D5), demand asserted with zero user contact + no kill criterion (Garry T., PMF), + ST5. Scored by `check-ai-product.py`. **5/5 in 3/3 cold runs, REBUILD ×3**, ST5 refused in all 12 isolated critic contexts; every run named the same structural blind spot (no ai-product lens owns the autonomous-agent security/liability surface → escalate to `trust`/`full`).
  - Runs recorded under `runs/2026-06-12-{aura-trust,sentry-ai-product}-{baseline,run2,run3}.md`; the two baselines are re-scored in CI. Both checkers' paraphrase corpora are added to `plugins-factory/evals/recall-corpus/` (45 paraphrases) and check-recall-asserted — the harness caught **5 brittle-pattern gaps on first run** ("kill criterion / can't be falsified", "a demo answers in an afternoon", "safety envelope… swap it", "unmeasurable/uncorrectable inferences", "deletion that doesn't delete"), now widened with every baseline re-scoring full. **All four product sub-councils with their own fixtures are now at N=3.** README + protocol + catch-rate tables updated.
