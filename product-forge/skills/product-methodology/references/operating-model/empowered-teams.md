...
- The two-sided empowerment-vs-alignment failure framing (over-direction → feature team; under-context → abandonment; both fixed by context + coaching) is a synthesis consistent with the book and with `product-operating-model.md`'s "empowerment is not a blank check"; the over/under-direction labels are this file's framing of Marty C.'s argument, not quoted terminology.
