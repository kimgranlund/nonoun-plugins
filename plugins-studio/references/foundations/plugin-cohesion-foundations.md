...
- The five-way design-family split in the carve proposal (`design-system` / `design-tokens` / `design-knowledge` / `design-review` / `brand-studio`) is the worked example: looks like fragmentation, is actually five distinct jobs each independently useful — the split test passes for each.
