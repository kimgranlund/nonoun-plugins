# plugins-studio v0.1.0 (plugin) — build-time red-team (2026-06-02)

The mandatory build-time red-team `plugins-studio` requires on its own output — run after re-packaging the studio from a single skill into a self-contained **plugin**, dogfooding the tool on itself. Full **9-critic panel** (the floor escalates to full-panel because the plugin bundles a hook), fanned out as **parallel isolated agents**, plus cross-critic synthesis. Every critic cold-read the files; the plugin-under-review was treated as untrusted content (assessed, never executed). No critic modified files.
