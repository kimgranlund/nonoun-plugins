...
- **agent-ops** — the v0.2 **`repo-memory` MCP** shipped 2026-06-11 (0.1.9): per-instance read-only retrieval over a repo's agent docs + the `.brain/audit-history/` ledger; mirrors the brand/product corpus MCPs, liveness + guard selftests in CI. **All three maker-class plugins now ship corpus/memory MCPs.** Remaining: first real-repo audit applications to validate the directional rubrics.
