...
- **Later**
  - **Council calibration — every council now has ≥2 fixture shapes (2026-06-10):** plugins-factory (mega-helper excess + docs-studio vacancy) · brand-forge (strategy/Northwind + design/Lumina + voice/Verve — all three sub-councils) · product-forge (strategy/Atlas + PRD/Pulse) · agent-ops (over-fleet/Nightshift + monolith/OmniDesk). The four mature fixtures are at N=3 (100% per-defect); the six newer shapes have recorded N=1 baselines, all REBUILD/BLOCKED, all ST5-refused, all CI-rescored. **Rubric calibration N≥3 reached** and **its four findings + the council's two blind spots were folded back** (rubric 0.2.0: AP-P6/AP-P7 + hard tests 11–13; eval-prompts 0.2.0: PF5/CF5) and proven by docs-studio. **The AP-P7 liveness finding is now a CI gate** (`check-mcp-liveness.py`). Remaining depth: promote the six N=1 baselines to N=3.
