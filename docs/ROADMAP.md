...
- **Later**
  - **Council calibration — complete on both axes 2026-06-10, then folded back:** all four councils at **N=3 with 100% per-defect catch-rates** (pf 2/2 ×3 · brand 6/6 ×3 · product 7/7 ×3 · agent-ops 8/8 ×3; trust boundary held in every run); **rubric calibration N≥3 reached** (three real carve targets, three discriminating verdicts). The four rubric findings + the council's two named blind spots were **folded back into the instruments** (rubric 0.2.0: AP-P6/AP-P7 + hard tests 11–13 + pre-carve note; eval-prompts 0.2.0: PF5/CF5) and **proven by a second fixture shape** (`docs-studio` — vacancy+deadness, caught 2/2 ×2, BLOCKED). Remaining depth: a CI liveness smoke gate (AP-P7 mechanization); more fixture shapes for the other three councils.
