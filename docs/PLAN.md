...
1. **Further sub-council shapes** — product-forge's `trust`/`ai-product` sub-councils (the remaining un-calibrated council shapes).
...
3. **Calibrate the instruments against reality** — apply `carve-quality.md` to a real carve (`empirical_applications: 0`); the rubric library at large still needs N≥3 real-plugin scorings.
