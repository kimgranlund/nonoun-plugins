...
- **First real-repo audit recorded** (agent-ops 0.1.10) — agent-ops's `repo-review` run cold on its own host repo (`reviews/2026-06-11-claude-plugins-audit.md`), moving its rubric directional → recorded. A real architectural review of the catalog: a tailored 9-dim rubric (3.7/5), a cascade-ranked 3-P0/3-P1/6-P2 backlog, a Tier-1 preservation contract, and an honest method-validation payload. **Acting on the audit backlog** (2 of 3 P0s done): **P0-2 DONE** — `gen-index.py selftest` (R-1 regression guard, proven to fail when the bug is reintroduced; CI-wired). **P0-1 DONE** — `repo-review` behavioral coverage: `agent-ops/evals/repo-review-calibration/` (a 6-planted-smell synthetic repo + checker), cold baseline **6/6 caught**, recall-gated, CI-rescored — the audit's headline doctrine gap closed. Remaining: P1-1 the `check-sourcing.py` fork (needs a judgment call — are the 246/98 forks correctly independent or is one stale?); P0-3 eval dedup (collapse the 9 forked checkers — note: P0-1 added one more checker, so the dedup's value rose).
