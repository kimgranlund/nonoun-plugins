...
### I-11 · P1/P6 — `bin/corpus-reader/` is off-job for the judge, and `context-cost.py` is blind to `bin/` payload · ~~blind-spot MECHANIZED 0.2.39; relocation is a tracked decision~~

Boris C. + Elon M. (Critical), Steve Y. + David F. (Major), grep-proven: no plugins-factory command/skill/agent routes to `bin/corpus-reader/`; it is the canonical source `sync-corpus-reader.py` fans into brand-forge + product-forge. The committed reader `lib/` + `demo-corpus` (~24 files / 111KB) ships; the gitignored fonts do **not** (the "licensing" framing was refuted). **RESOLVED — the gate blind spot (the inherited defect, 0.2.39):** `context-cost.py` now measures committed (git-tracked) `bin/` **install payload** as a distinct axis from always-on context (bin/ files never load into context, so this is distribution weight, not a token tax — a distinction the council blurred), reports it per plugin, and **WARNs when a single bin/ subtree dominates install weight** (`bin/corpus-reader` trips it: plugins-factory 43%, brand-forge 80%, product-forge 73% — the off-job mass is now visible to the gate that was structurally blind to it). git-tracked-only keeps it clean-clone-true (the gitignored fonts excluded). **Still open — the relocation (a design decision, candidate D-14):** the canonical reader living in plugins-factory (which doesn't use it) is a cohesion smell, but the target is a genuine fork — (i) a new repo-root `tools/` shared-tooling dir (breaks "everything is a plugin"), (ii) make it on-job by giving plugins-factory a corpus command, (iii) move canonical ownership to a maker plugin (asymmetric), or (iv) accept it now that the weight is measured + flagged. Now that the cost is visible, this is deliberate, not urgent.
