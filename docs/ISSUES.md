...
### I-10 · P7/P3 — cross-plugin critic-agent name collision (the meta-tool ships the scale defect it red-teams others for)

Steve Y. (confirmed by `git`): `critic-boris-c`, `critic-andrej-k`, `critic-simon-w` carry byte-identical frontmatter `name:` across **plugins-factory + agent-ops**; `critic-garry-t` across **agent-ops + product-forge**. The `plugin-council` orchestrator dispatches critics by **bare name** (`plugin-council.md:45`), and `validate_plugin.py`'s collision check is scoped to `commands ∩ skills` **within one plugin** — blind to cross-plugin agent collisions. Severity is *contingent on the runtime's bare-name resolution* (namespace-local → no live ambiguity; global → a nondeterministic roster when two council plugins are co-enabled, which is the maintainer's own auto-enabled state). **Fix (two parts, both-or-neither):** (a) a `validate_plugin` marketplace-mode agent-`name` uniqueness assertion; (b) namespace the slugs (`critic-pf-*` / `critic-ops-*`) — but (b) touches the deliberate obscured-`critic-<first>-<initial>` convention catalog-wide, and (a) fails CI until (b) lands.
...
### I-12 · P9 — the executing gates' "trusted-only" boundary is prose, not a mechanism

Simon W. (Critical), Elon M. (Major), confirmed: `check-mcp-liveness.py` and `context-cost.py --with-mcp` spawn a target plugin's MCP server (full `os.environ` passed through) guarded by **only a docstring** ("TRUSTED catalog plugins only", `check-mcp-liveness.py:12`) — no trust flag, no allowlist, no refusal-by-default. The catalog's own evaluation tool holds the lethal trifecta it grades Score-1 in others; the natural motion ("run the gates against this bundle I'm vetting") is the exploit. Bounded today by *who* runs it (maintainer/CI; the council itself is correctly tool-locked and never executes these). **Fix:** refuse by default; require an explicit per-invocation trust assertion (a `--trusted-source` flag **and** a CI-set env marker, or a path-under-declared-catalog-root check); CI sets it, a human almost never would.

### I-13 · P8 — the judge keeps no durable record, and the council's recall is unmeasured

Charity M. (Critical) + Andrej K. (C2/C3): `score`/`promote` print verdicts to chat and persist nothing; `scores/<plugin>.json` is specced (`rubric-manifest.json`) but unshipped; `empirical_applications: 0` across all rubrics; one `reviews/` entry before this one. The council's catch-rate is calibrated on two confessional fixtures (`mega-helper`, `docs-studio`), N=1 per defect-shape, never a real third-party plugin. **Fix (already ROADMAP'd, `plugins-factory/ROADMAP.md:10` + PLAN #4):** ship the `scores/<plugin>.json` writer (to `${CLAUDE_PLUGIN_DATA}`, not the cache root); re-source recall corpora from real `runs/*.md` transcripts (the deeper half of the 0.2.35 honesty fold); score one real third-party plugin cold to move `empirical_applications` off 0.
